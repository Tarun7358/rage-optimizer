...
// Stat mini card
function StatCard({ icon: Icon, label, value, color = 'text-accentRed', suffix = '', index }) {
  return (
    <motion.div 
      ...
      whileHover={{ y: -3 }}
      className="glass-card rounded-2xl p-4.5 flex flex-col items-center text-center hover:border-accentRed/35 hover:shadow-neonGlow"
    >
      <div className="p-2 bg-white/5 dark:bg-white/3 rounded-xl mb-2.5">
        <Icon className={`w-5 h-5 ${color} opacity-90`} />
      </div>
      <span className={`text-xl sm:text-2xl font-gaming font-black tracking-wide ${color}`}>{value}{suffix}</span>
      <span className="text-[9px] text-textGray font-bold uppercase tracking-widest mt-1.5">{label}</span>
    </motion.div>
  );
}
    ...
    return (
      <div className="min-h-screen bg-darkBg flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <span className="text-textGray text-sm font-semibold tracking-wider animate-pulse">Loading Console...</span>
        </div>
      </div>
    );
  }
      ...
      <main className="flex-1 ml-0 lg:ml-64 p-4 sm:p-6 md:p-8 overflow-y-auto transition-all duration-300">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-borderColor pb-5">
            <div>
              <span className="text-[10px] text-accentRed font-bold uppercase tracking-[0.2em]">Console Panel</span>
              <h1 className="text-2xl sm:text-3xl font-gaming font-black text-white mt-1 uppercase tracking-wider">{guildName}</h1>
              <p className="text-textGray text-xs mt-1">Manage your bot configurations and guard logs in real-time.</p>
            </div>

            <button
              onClick={saveSettings}
              disabled={saveStatus === 'saving'}
              className={`flex items-center space-x-2 px-5 py-3 rounded-xl font-gaming font-black tracking-wider text-xs transition-all self-start sm:self-auto cursor-pointer shadow-sm ${
                saveStatus === 'saved'
          ...
          {liveStats && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard icon={Server} label="Total Servers" value={liveStats.servers} color="text-accentRed" index={0} />
              <StatCard icon={Users} label="Total Members" value={liveStats.members.toLocaleString()} color="text-blue-400" index={1} />
              <StatCard icon={Wifi} label="Bot Latency" value={liveStats.ping} suffix="ms" color={liveStats.ping < 100 ? 'text-emerald-400' : 'text-yellow-400'} index={2} />
              <StatCard icon={Clock} label="Gateway Uptime" value={formatUptime(liveStats.uptimeSeconds)} color="text-purple-400" index={3} />
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="glass-card rounded-2xl overflow-hidden shadow-md">
              <div className="flex items-center space-x-3 px-5 py-4 border-b border-borderColor">
                ...
                <div>
                  <h3 className="font-gaming font-bold text-white text-xs sm:text-sm uppercase tracking-wider font-gaming">Active Control Systems</h3>
                  <p className="text-[9px] text-textGray">Toggle modules to adjust server features.</p>
                </div>
              </div>
                ...
                {modules.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className={`flex items-center justify-between p-3.5 rounded-xl border transition-all ${
                      isEnabled ? 'bg-accentRed/5 border-accentRed/20' : 'bg-white/3 border-borderColor/60'
                    }`}>
                      <div className="flex items-center space-x-3 pr-2">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                          isEnabled ? 'bg-accentRed/15 border border-accentRed/25' : 'bg-white/5 border border-borderColor'
                        }`}>
                          <Icon className={`w-4 h-4 ${isEnabled ? 'text-accentRed' : 'text-textGray'}`} />
                        </div>
                        <div>
                          <label className="text-xs sm:text-sm font-semibold text-white block">{item.label}</label>
                          <span className="text-[9px] sm:text-[10px] text-textGray leading-normal block">{item.desc}</span>
                        </div>
                      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card rounded-2xl overflow-hidden shadow-md">
                <div className="flex items-center space-x-3 px-5 py-4 border-b border-borderColor">
                  ...
                  <div>
                    <h3 className="font-gaming font-bold text-white text-xs sm:text-sm uppercase tracking-wider font-gaming">Server Configuration</h3>
                    <p className="text-[9px] text-textGray">Current metadata snap.</p>
                  </div>
                </div>
                <div className="p-5">
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-white/4 border border-borderColor rounded-xl p-4 text-center">
                      <span className="block text-xl sm:text-2xl font-gaming font-black text-white tracking-wide">
                        {settings.welcome?.autoRoles?.length || 0}
                      </span>
                      <span className="text-[8px] sm:text-[9px] text-textGray uppercase font-bold tracking-wider">Auto-Roles</span>
                    </div>
                    <div className="bg-white/4 border border-borderColor rounded-xl p-4 text-center">
                      <span className="block text-xl sm:text-2xl font-gaming font-black text-white tracking-wide">
                        {settings.moderation?.badWords?.length || 0}
                      </span>
                      <span className="text-[8px] sm:text-[9px] text-textGray uppercase font-bold tracking-wider">Banned Words</span>
                    </div>
                    <div className="bg-white/4 border border-borderColor rounded-xl p-4 text-center">
                      <span className="block text-xl sm:text-2xl font-gaming font-black text-white tracking-wide">
                        {discordMeta.channels.filter(c => c.type === 0).length}
                      </span>
                      <span className="text-[8px] sm:text-[9px] text-textGray uppercase font-bold tracking-wider">Channels</span>
                    </div>
                    <div className="bg-white/4 border border-borderColor rounded-xl p-4 text-center">
                      <span className="block text-xl sm:text-2xl font-gaming font-black text-white tracking-wide">
                        {discordMeta.roles.length}
                      </span>
                      <span className="text-[8px] sm:text-[9px] text-textGray uppercase font-bold tracking-wider">Roles</span>
                    </div>
                  </div>

                  {/* Permission warning */}
                  <div className="p-3.5 rounded-xl bg-amber-500/5 border border-amber-500/25 flex items-start space-x-3">
                    <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                    <p className="text-[10px] sm:text-[11px] text-amber-500 leading-relaxed font-medium">
                      <strong>Important:</strong> Keep the <span className="font-mono bg-amber-500/10 px-1 rounded border border-amber-500/20">RAGE OPTIMIZER</span> role at the top of your role hierarchy for ban/kick to work.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card rounded-2xl overflow-hidden shadow-md">
                <div className="flex items-center justify-between px-5 py-4 border-b border-borderColor">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="font-gaming font-bold text-white text-xs sm:text-sm uppercase tracking-wider font-gaming">Live Security Activity</h3>
                      <p className="text-[9px] text-textGray">Real-time socket events</p>
                    </div>
                  </div>
                ...
                <div className="p-4">
                  {activity.length === 0 ? (
                    <div className="text-center py-8">
                      <Activity className="w-8 h-8 text-textGray/20 mx-auto mb-2" />
                      <p className="text-xs text-textGray/45 italic">Listening for live websocket events...</p>
                    </div>
                  ) : (
                        ...
                        {activity.map((item, i) => {
                          ...
                          return (
                            <motion.div
                              ...
                              className={`flex items-start space-x-3 p-3 rounded-xl border ${cfg.bg}`}
                            >
                              <Icon className={`w-3.5 h-3.5 ${cfg.color} mt-0.5 shrink-0`} />
                              <div className="flex-1 min-w-0">
                                <p className="text-xs text-white/90 leading-snug truncate">{item.msg}</p>
                                <p className="text-[9px] text-textGray/50 mt-1 font-medium">
          ...
          {liveStats && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="glass-card rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md"
            >
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <div className="text-sm font-bold text-white font-gaming">{liveStats.botTag}</div>
                  <div className="text-[10px] text-textGray">
                    {liveStats.isLive ? 'Connected to Discord Gateway API v10' : 'Running in Mock Mode'}
                  </div>
                </div>
              </div>
              <div className="flex items-center space-x-4 text-[10px] sm:text-xs">
                <div className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-full border ${
                  ...
                  <span className="font-bold tracking-wider">{liveStats.isLive ? 'LIVE' : 'MOCK'}</span>
                </div>
                <div className="text-textGray font-semibold">
                  Serving <span className="text-white font-bold">{liveStats.servers}</span> Guild{liveStats.servers !== 1 ? 's' : ''}
                </div>
                <div className="text-textGray font-semibold">
                  Latency: <span className={liveStats.ping < 100 ? 'text-emerald-400 font-bold' : 'text-yellow-400 font-bold'}>{liveStats.ping}ms</span>
                </div>
              </div>
            </motion.div>
          )}
